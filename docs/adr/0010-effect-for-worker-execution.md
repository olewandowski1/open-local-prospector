...
The React presentation layer remains conventional Next.js code. Thin Route Handlers may execute server effects at a single boundary, while UI state and rendering do not adopt Effect abstractions. Drizzle and `better-sqlite3` remain the persistence implementation behind Effect-wrapped repositories. Effect remains pinned to stable v3; any major-version migration requires a deliberate compatibility review rather than following prereleases.
