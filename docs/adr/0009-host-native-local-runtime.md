...
The v1 application runs as a host-native Next.js web process and a host-native background worker started by one project command. This lets small runtime adapters invoke already-installed Codex and Claude Code command-line clients without mounting credential directories or bridging host executables into containers. Provider login remains an explicit terminal action outside the application.
