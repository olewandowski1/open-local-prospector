...
2. The first-run checklist verifies SQLite, Brave Search, Playwright, disk space, and provider readiness.
...
- **Brave Search API:** only first-slice discovery connector, behind `DiscoverySource`.
...
Background worker -------------------------+
    |             |              |
    v             v              v
Brave Search   Playwright     RuntimeAdapter
...
- **Brave Search:** API key from ignored `.env.local`; never exposed client-side or to AI runtimes.
...
- First-run readiness and Brave Search configuration.
...
- Brave Search discovery, identity corroboration, deduplication, and eligibility exclusion.
...
| Brave Search coverage misses local businesses | Low yield in some locations/categories | Search exhaustion state, query audit trail, later `DiscoverySource` adapters |
...
V1 is complete only when Oliver can configure Brave Search and an authenticated local runtime, execute and resume a bounded run, receive deduplicated evidence-backed candidates, review and correct every material assertion, export unsuppressed results, restart without losing completed work, and pass the versioned evaluation fixtures without Docker, cloud hosting, or usage-based AI APIs.
