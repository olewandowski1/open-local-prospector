...
- The runtime selector supports ready Codex and Claude Code adapters, remembers the previous choice, and shows readiness.
...
- **Subscription runtime web search:** the selected Codex or Claude Code CLI is the only discovery connector, behind `DiscoverySource`; no separate search API key is required.
...
- **Runtime adapters:** minimal local subprocess adapters for Codex and Claude Code non-interactive modes.
...
7Ovr is an optional design accelerator, not a second component system. Blocks are installed through the shadcn registry, inspected, and adapted to the product domain. Canonical shadcn primitives, semantic theme tokens, local assets, accessibility, responsive behavior, and repository conventions remain authoritative; third-party block code is never accepted without review. Project-scoped shadcn MCP configuration is provided for Codex and Claude Code-compatible clients.
...
- Codex runtime adapter first, then the Claude Code adapter behind the same contract.
